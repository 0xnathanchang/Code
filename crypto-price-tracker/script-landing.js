...
// Changed to fetch top 20 coins
const TOP_COINS_PARAMS = '?vs_currency=usd&order=market_cap_desc&per_page=20&page=1&sparkline=false&price_change_percentage=24h';

// Custom coins you want to always include (even if not in top 20)
// Added Bitcoin, Ethereum, Solana, Binance Coin (BNB), and EDU Coin
const CUSTOM_COINS = ['bitcoin', 'ethereum', 'solana', 'binancecoin', 'edu-coin']; // Ensure these IDs are correct for CoinGecko
...
document.addEventListener('DOMContentLoaded', function() {
    setupLandingPage(); // Call this first to decide what to show
    setupEventListeners();
});

// Set up initial landing page display
function setupLandingPage() {
    landingPage.classList.remove('hidden'); // Ensure landing page is visible
...
function handleGetStarted() {
    ...
    loadPortfolioFromStorage(); // Load portfolio data
    loadCryptoData(); // Load crypto data for the main app
...
function createCryptoCard(crypto) {
    ...
    // Determine if it's one of the explicitly requested custom coins for the "Featured" badge
...
function hideLoading() {
    ...
    // Ensure cryptoGrid is visible when not loading, but only if main content is active
